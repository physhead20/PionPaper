...
TH1D *hdataPionInitialMomentum = new TH1D("hdataPionInitialMomentum", "Pion Initial Momentum (MeV)", 500, 0, 2500); //This is the initial Kin En
...
TH2D *hdataPiondEdXvsRR1 = new TH2D("hdEdxVSrr_contained", "dE/dX vs Residual Range - contained",200, 0, 100, 200, 0, 50);
...
//for (Long64_t jentry=0; jentry<20000;jentry++)  
   ...
   for(size_t nTPCtrk = 0; nTPCtrk < ntracks_reco; nTPCtrk++)
      ...
      {ExitingTrack = true;
        ...
        if(!ExitingTrack){
           //std::cout <<"Matched track " << nTPCtrk << "Track PIDA " << trkpida[nTPCtrk][1] << std::endl;
  ...
  // ### Filling the initial kinetic energy plot ###
   ...
   for(size_t nTPCtrk = 0; nTPCtrk < ntracks_reco; nTPCtrk++)
	 ...
	 // ### [trk number][plane 0 = induction, 1 = collection][spts number] ###
      ...
      // ############ FIXING THE TRACK CALORIMETRY ISSUES!!!! #######
...
for(size_t nTPCtrk = 0; nTPCtrk < ntracks_reco; nTPCtrk++)
   ...
   if(!MatchTPC_WVTrack[nTPCtrk]){continue;}
       ...
       // ###################################################
           // ### Looping over the spacepoints for this track and reordeirng ###
           // ###################################################
           size_t dimCalo = 0;
           dimCalo = nPionSpts;
	     ...
	     if(Pionresrange[caloPoints] > Pionresrange[caloPoints-1]){
           ...
           PIDAEfin->Fill(trkpida[nTPCtrk][1],kineticEnergy);
           PIDAEin->Fill(trkpida[nTPCtrk][1],InitialKinEnAtTPC);
...
std::cout << "Crossing particles " << nCrossing << std::endl;
    
 std::cout << "Stoppin particles " << stopping << std::endl;   
//std::cout << "Cool things from AG --> StopMu (in TPC): " << AGstopMu << " CrossMu:" << AGcrossMu << " Electrons:" <<AGelectron << " Pions: " << AGpion <<"  StopPi:" << AGstopPI << endl;
...
// ###############################################
TFile myfile("PionXSection_Irene_histos.root","RECREATE");
